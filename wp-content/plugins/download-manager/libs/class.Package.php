...
class Package {
    ...
    function Prepare($ID = null, $template = null, $template_type = 'page')
    {
        ...
        if(isset($data['terms_lock']) && $data['terms_lock'] != 0 && (!function_exists('wpdmpp_effective_price') || wpdmpp_effective_price($post_vars['ID']) ==0) && $limit_over == 0){
            ...
            $post_vars['download_link_extended'] = "<div class='panel panel-default card card-default terms-panel' style='margin: 0'><div class='panel-heading card-header'>{$data['terms_title']}</div><div class='panel-body card-body' style='max-height: 200px;overflow: auto'>{$data['terms_conditions']}</div><div class='panel-footer card-footer'><label class='m-0'><input type='checkbox' class='wpdm-checkbox wpdm-lock-terms' data-target='.download_footer_{$post_vars['ID']}' > {$data['terms_check_label']}</label></div><div class='panel-footer card-footer  download_footer_{$post_vars['ID']}' style='display:none;'>{$post_vars['download_link_extended']}</div></div>"; // onclick='jQuery(\".download_footer_{$post_vars['ID']}\").slideToggle();'
...
//isset($package['password_lock']) && (int)$package['password_lock'] == 1 &&
        ...
        if (isset($package['captcha_lock']) && (int)$package['captcha_lock'] == 1) {
            ...
            $data .= "<div class='panel panel-default card card-default'><div class='panel-heading card-header'>".__("Verify CAPTCHA to Download",'download-manager')."</div><div class='panel-body card-body wpdm-social-locks text-center'>{$captcha}</div></div>";
        }
        ...
        if($sociallock!=""){
            $data .= "<div class='panel panel-default card'><div class='panel-heading card-header'>".__("Download",'download-manager')."</div><div class='panel-body card-body wpdm-social-locks text-center'>{$sociallock}</div></div>";
        }
